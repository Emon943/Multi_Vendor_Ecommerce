...
if(isset($_POST['inst'])){
	...
	mysqli_query($con,"insert into product(`title`,`s_cat`,`loc`,`short_des`,`brand`,`price`,`user`,`img1`,`img2`,`img3`) values('$title',$cat,$loc,'$des','$brand',$price,$user,'$f_insnt1','$f_insnt2','$f_insnt3')");
...
if(isset($_POST['edit'])){
	...
	mysqli_query($con,"update product set title='$title',s_cat=$cat,loc=$loc,short_des='$des',brand='$brand',price=$price,img1='$f_insnt1',img2='$f_insnt2',img3='$f_insnt3' where id=$id");
	
	$msg="Product $title updated Successfully!";
}

$pro=mysqli_query($con,"select * from product where status=1");
?>
...
<?php if($msg!= NULL) { ?>
  ...
  <div class="inner-box">
                <div class="col-sm-12 page-content">
                        ...
                        <h2 class="title-2"><i class="fa fa-cube"></i> Live Products </h2>
													  ...
													  <div class="form-group">
														...
														<div class="col-sm-10">
														  <select type="text" class="form-control" name="user" required>
																...
																<?php 	$categ=mysqli_query($con,"select * from user where type=1");
																		
																		for($i=0; $i<mysqli_num_rows($categ); $i++){ 
																		$row3=mysqli_fetch_array($categ,MYSQLI_ASSOC);?>
																<option value="<?php echo $row3["id"] ?>"><?php echo $row3["company"] ?></option>
																		<?php } ?>
														  </select>
														</div>
													  </div>
                                    ...
                                    <td style="width:10%" class="action-td">
                                        <div>
										  ...
										  <div class="modal-dialog" role="document">
											...
											<p><a class="btn btn-info btn-xs" href="change?id=<?php echo $row_p["id"]; ?>&value=0&page=live_product"> <i class="fa fa-gift"></i> Archive </a>
                                            </p>
											
											<p><a href="product?id=<?php echo $row_p["id"]; ?>" class="btn btn-default btn-xs" target="_blank"> <i class=" fa fa-download"></i> Export
                                            </a></p>
											
                                           <p><a href="del?id=<?php echo $row_p["id"]; ?>&page=live_product&field=product" class="btn btn-danger btn-xs"> <i class=" fa fa-trash"></i> Delete
                                            </a></p>
                                        </div>
                                    </td>
