...
if(isset($_GET['status']) && $_GET['status']==0){
	$log=mysqli_query($con,"select * from invoice where status=0 order by id DESC");
}
else if(isset($_GET['status']) && $_GET['status']==1){
	$log=mysqli_query($con,"select * from invoice where status=1 order by id DESC");
}
else if(isset($_GET['status']) && $_GET['status']==2){
	$log=mysqli_query($con,"select * from invoice where status<>0 and status<>1 order by id DESC");
}
else
$log=mysqli_query($con,"select * from invoice order by id DESC");
?>
   <div class="inner-box">
